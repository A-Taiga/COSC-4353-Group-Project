...
export const calculatePrice = (req: Request, res: Response) => {
    try {
        ...
        if (gallonsRequested < 0) {
            return res.status(400).json({ message: 'Gallons requested cannot be negative' });
        }
        ...
        console.error('Error calculating price:', error); // Move this line inside the try block
